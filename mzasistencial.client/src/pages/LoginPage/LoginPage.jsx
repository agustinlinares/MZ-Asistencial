...
const LoginPage = () => {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (res.status === 429) {
                // Usuario bloqueado
                dispatch({ type: 'LOGIN_ERROR', message: data.error, toastType: "error" });
            ...
            if (!res.ok) {
                ...
                if (data.requiereCaptcha && data.captchaId) {
                    dispatch({ type: 'SET_CAPTCHA', captchaId: data.captchaId, message: data.error });
                } else if (data.requiereCaptcha) {
                    // Ya tenía captcha, refrescar imagen
                    const nuevoId = data.captchaId || captchaId;
                    dispatch({ type: 'REFRESH_CAPTCHA', captchaId: nuevoId });
                    dispatch({ type: 'LOGIN_ERROR', message: data.error, toastType: "warning" });
                } else {
                    dispatch({ type: 'LOGIN_ERROR', message: data.error || 'Usuario o contraseña incorrectos.' });
                }
                return;
            }
